...
def generateCode():
    try:
        ...
        qr = qrcode.QRCode(
            version=int(size.get()),  # Convert size to integer
            box_size=10,
            border=5
        )
        ...
        fileDirec = os.path.join(loc.get(), name.get() + '.png')  # Getting the directory where the file has to be saved
        img.save(fileDirec)  # Saving the QR Code
